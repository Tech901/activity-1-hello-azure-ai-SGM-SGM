...
def classify_311_request(request_text: str) -> dict:
    ...
    response = client.chat.completions.create(
        ...
        temperature=0,
        messages=[
            {"role": "system", "content": "Classify the complaint into exactly one of: Pothole, Noise Complaint, Trash/Litter, Street Light, Water/Sewer, Other. Return JSON with keys: category, confidence (0.0-1.0), reasoning."},
...
def check_content_safety(text: str) -> dict:
    ...
    return {"safe": all(s == 0 for s in categories.values()), "categories": categories}
